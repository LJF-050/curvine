...
/// Authoritative origin of a load/export job. This is a real, journaled field on
/// the job (not a UI heuristic) so that callers can reliably distinguish a job
/// submitted explicitly by a user from one auto-generated by fs_mode export.
#[derive(
    Clone,
    Copy,
    Debug,
...
impl JobSourceType {
    /// Stable machine-readable identifier exposed over HTTP/CLI.
    pub fn as_str(&self) -> &'static str {
        match self {
            JobSourceType::Manual => "manual",
            JobSourceType::FsModeAuto => "fs_mode_auto",
        }
    }
}
...
pub struct JobStatus {
    ...
    pub running_files: u64,
    pub pending_files: u64,
    pub loading_files: u64,
    pub source_type: JobSourceType,
    pub tasks: Vec<LoadTaskStatus>,
}
...
pub struct LoadJobCommand {
    ...
    /// Origin of the job. `None` is treated as [`JobSourceType::Manual`] for
    /// backward compatibility with older clients that do not set the field.
    #[serde(default)]
    pub source_type: Option<JobSourceType>,
}
...
pub struct LoadJobCommandBuilder {
    ...
    source_type: Option<JobSourceType>,
}
...
impl LoadJobCommandBuilder {
    ...
    pub fn source_type(mut self, source_type: JobSourceType) -> Self {
        let _ = self.source_type.insert(source_type);
        self
    }
